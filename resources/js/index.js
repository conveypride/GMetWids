...
dropdownItems.forEach(function(item) {
    item.addEventListener('click', function() {
      ...
      window.location.href = "/" + selectedDistricted + "?click=daily";
...
document.getElementById("selectedDist").addEventListener("change", function() {
  ...
  if(citynull != "null"){
    window.location.href = "/" + cityName + "?click=cities";
  }
});
...
console.log(markerdata);
  ...
  if(polygondata){
    polygondata.forEach(obj => {  map.addLayer({
      'id': obj.polygonId,
      'type': 'fill',
      'source': 'polygons',
      'paint': {
